    ...
    <div class="page-header">
        <div class="container-xl">
            ...
            <div class="flex flex-row justify-start items-center mt-3 gap-3">
                <button
                    class="addons_filter text-[#2B2F37] bg-[--lqd-header-search-bg] font-medium text-[13px] rounded-[55px] px-2 py-1 cursor-pointer bg-none border-none transition-all [&.active]:bg-[--tblr-primary] [&.active]:text-white active"
                    data-filter="All">ALL</button>
                <button
                    class="addons_filter text-[#2B2F37] bg-[--lqd-header-search-bg] font-medium text-[13px] rounded-[55px] px-2 py-1 cursor-pointer bg-none border-none transition-all [&.active]:bg-[--tblr-primary] [&.active]:text-white"
        ...
        <div class="container-xl">
            <div class="flex flex-wrap justify-start relative">
                @foreach ($extensions as $extension)
                    <div class="extension rounded-[20px] border-[1px] border-[#f3f3f3] w-full border-solid m-2 flex items-center p-[20px] relative"
                        data-price="{{ $extension->price }}" data-installed="{{ $extension->installed }}"
                        data-name="{{ $extension->name }}">
                        <div class="img_tag w-[53px] h-[53p] rounded-[10px] mr-[30px]">
                        ...
                        <div class="absolute right-[20px] flex items-center">
                            ...
                            <div
                                class="btn_installed mx-[20px] bg-[#330582] w-[50px] h-[50px] rounded-[55px] flex items-center justify-center {{ $extension->installed == 1 ? '' : 'hidden' }}">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <mask id="mask0_3091_21566" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0"
                                        y="0" width="24" height="24">
                                        <rect width="24" height="24" fill="#D9D9D9" />
                                    </mask>
                                    <g mask="url(#mask0_3091_21566)">
                                        <path
                                            d="M9.55 18L3.85 12.3L5.275 10.875L9.55 15.15L18.725 5.97501L20.15 7.40001L9.55 18Z"
                                            fill="white" />
                                    </g>
                                </svg>
                            </div>
                            <div class="btn_install mx-[20px] bg-white w-[50px] h-[50px] rounded-[55px] flex items-center justify-center {{ $extension->installed == 0 ? '' : 'hidden' }}"
                                data-name="{{ $extension->name }}">
                                <svg class="svg_install" width="54" height="54" viewBox="0 0 54 54"
                                    fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <circle cx="27" cy="27" r="26.5" fill="white" stroke="#F8F8F8" />
                                    <path d="M26 36V28H18V26H26V18H28V26H36V28H28V36H26Z" fill="#1C1B1F" />
                                </svg>
                                <svg class="svg_loading hidden animate-spin" width="24" height="24" viewBox="0 0 24 24"
                                    fill="none" xmlns="http://www.w3.org/2000/svg">
