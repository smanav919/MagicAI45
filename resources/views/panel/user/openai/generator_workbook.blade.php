                            ...
                            <div class="mb-3 col-xs-12 @if($question->type == 'rss_feed') relative @endif">
                                ...
                                @elseif($question->type == 'rss_feed')
                                    ...
                                    <input type="{{ $question->type }}" class="form-control" id="{{ $question->name }}"
                                        name="{{ $question->name }}" maxlength="{{ $setting->openai_max_input_length }}"
                                        placeholder="{{ __($placeholder) }}" required="required">
                                    <div class="fetch-rss absolute right-0 bottom-[1px] leading-[45px] pr-6 pl-4 border-l border-r-0 border-b-0 border-t-0 border-solid border-[var(--tblr-border-color)] cursor-pointer hover:scale-95">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M20 11a8.1 8.1 0 0 0 -15.5 -2m-.5 -4v4h4" /><path d="M4 13a8.1 8.1 0 0 0 15.5 2m.5 4v-4h-4" /></svg>
                                        {{__('Fetch RSS')}}
                                    </div>
                    ...
                    @else
                        ...
                        @if ($setting->hosting_type == 'high')
                            ...
                            const eventSource = new EventSource("/dashboard/user/openai/generate?message_id=" +
                                ...
                                .creativity @if ($openai->type == 'youtube') +"&youtube_url="+$("#url").val() @endif @if ($openai->type == 'rss') +"&rss_image="+$("#title :selected").attr('data-image') @endif );
