...
<div
    ...
    <div class="flex gap-2">
        <div class="flex max-md:hidden">
            <label class="form-check form-switch mx-2 mt-2">
        ...
        <div class="flex gap-2">
            <div class="inline-flex flex-row items-center justify-center group" id="show_export_btns">
                <button class="bg-transparent border-none text-heading">
                ...
                <div id="export_btns"
                    class="invisible absolute end-0 top-0 flex translate-y-2 scale-95 flex-row items-center justify-center opacity-0 transition-all group-hover:!visible group-hover:translate-y-0 group-hover:scale-100 group-hover:!opacity-100">
            ...
            <div class="self-center lqd-chat-mobile-sidebar-trigger">
                <button class="w-8 h-8 grid-flow-row p-0 btn group place-items-center sm:hidden max-sm:grid"
                    type="button" data-lqd-toggle-class="lqd-is-active"
                    data-lqd-toggle-target="#chats-list-container">
                    <svg class="col-start-1 row-start-1 transition-all group-[&.lqd-is-active]:scale-75 group-[&.lqd-is-active]:rotate-45 group-[&.lqd-is-active]:opacity-0"
                        width="20" height="20" viewBox="0 0 24 24" fill="currentColor" stroke-width="2"
                        xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M2 12a2 2 0 1 1 4 0 2 2 0 0 1-4 0z" />
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M10 12a2 2 0 1 1 4 0 2 2 0 0 1-4 0z" />
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M18 12a2 2 0 1 1 4 0 2 2 0 0 1-4 0z" />
                    </svg>
                    <svg class="col-start-1 row-start-1 transition-all opacity-0 -rotate-45 group-[&.lqd-is-active]:rotate-0 group-[&.lqd-is-active]:!opacity-100"
...
<div class="conversation-area flex grow flex-col justify-between overflow-y-auto max-md:max-h-[70vh] max-sm:max-h-full"
    ...
    <form
        class="sticky bottom-0 z-10 flex w-full items-end gap-3 self-end bg-[--tblr-body-bg] p-8 py-[1.5rem] max-sm:items-end max-md:p-4 max-sm:p-3"
        id="chat_form">
        <input type="hidden" value="{{ $category->id }}" id="category_id">
        <input type="hidden" value="{{ $chat->id }}" id="chat_id">
        <div class="form-control flex flex-col min-h-[52px] rounded-[26px] p-0 max-sm:min-h-[45px]">
            <div class="flex hidden max-h-[120px] flex-wrap overflow-scroll p-[10px]" id="chat_images">

            </div>
            <div class="flex hidden max-h-[120px] flex-wrap overflow-scroll p-[10px]" id="chat_pdfs">

            </div>
            <hr class="hidden w-full split_line border-1" />
            <div class="relative flex items-center grow">
                <input type="file" id="selectImageInput" style="display: none;"
                    accept="image/*, application/pdf" />
                <button
                    class="lqd-chat-mobile-options-trigger btn text-heading collapsed h-8 w-8 shrink-0 origin-center bg-transparent p-0 mt-[3px] shadow-none transition-transform sm:hidden [&:not(.collapsed)]:rotate-45"
                    data-bs-toggle="collapse" data-bs-target="#chat-options" type="button"
                    aria-controls="chat-options">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                        aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2"
                        stroke-linecap="round" stroke-linejoin="round" class="ai ai-Plus">
                        <path d="M12 20v-8m0 0V4m0 8h8m-8 0H4" />
                    </svg>
                    <span class="sr-only">{{ __('Options') }}</span>
                </button>
                <textarea
                    class="text-heading m-0 w-full border-none bg-transparent !py-3 pe-[100px] ps-16 outline-none focus:border-none focus:ring-0 max-sm:pe-2 max-sm:ps-0 max-sm:text-[16px] max-sm:max-h-[200px]"
                    placeholder="{{ __('Type a message') }}" name="prompt" id="prompt" rows="1"></textarea>
                <div
                    class="pointer-events-none absolute bottom-0 end-2 start-2 flex items-end justify-between py-[5px] text-sm max-sm:static">
                    <div id="chat-options"
                        class="flex items-center justify-between grow max-sm:absolute max-sm:!-end-[calc(52px+1rem)] max-sm:!bottom-full max-sm:-start-2 max-sm:mb-3 max-sm:flex-col max-sm:items-start max-sm:gap-4 max-sm:rounded-xl max-sm:bg-[--tblr-body-bg] max-sm:px-4 max-sm:py-0 max-sm:shadow-lg sm:!flex sm:h-full max-sm:[&:not(.collapsing):not(.show)]:hidden">
                        <div class="flex items-center pointer-events-auto max-sm:pt-4">
                            <button type="button"
                                ...
                                <svg class="max-sm:w-5" width="11" height="21" viewBox="0 0 11 21"
                        ...
                        <div
                            class="flex items-center pointer-events-auto sm:ms-auto max-sm:flex-col max-sm:items-start max-sm:gap-4 max-sm:pb-4">
                            <button type="button" id="prompt_library"
                                ...
                                <svg class="max-sm:w-5" width="19" height="20" viewBox="0 0 19 20"
                                    fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                    <path
                    ...
                    <div class="pointer-events-auto max-sm:absolute max-sm:bottom-[10px] max-sm:end-2">
                        <button type="button" id="voice_record_button" title={{ __('Record audio') }}
                            class="text-heading flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center gap-2 rounded-full border-none bg-[transparent] p-0 transition-all hover:bg-[--lqd-pink] hover:text-black max-sm:h-auto max-sm:w-auto max-sm:!bg-transparent">
                            <svg aria-hidden="true" class="max-sm:w-5 max-sm:h-5" width="17" height="23"
                                viewBox="0 0 17 23" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                <path
                        ...
                        <button type="button" id="voice_record_stop_button" title={{ __('Stop recording') }}
                            ...
                            <svg aria-hidden="true" class="max-sm:w-5 max-sm:h-5" width="20" height="22"
        ...
        @if ($setting->hosting_type == 'high')
            <button
                @if ($app_is_demo &&
                        ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf' || $category->slug == 'ai_chat_image')) onclick="return toastr.info('This feature is disabled in Demo version.')" @endif
                class="btn btn-primary h-[52px] w-[52px] shrink-0 rounded-full p-0 max-sm:h-10 max-sm:w-10"
                @if ($app_is_demo &&
                        ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf' || $category->slug == 'ai_chat_image')) @else id="send_message_button" @endif form="chat_form">
        ...
        @else
            ...
            <button
                ...
                class="btn btn-primary h-[52px] w-[52px] shrink-0 rounded-full p-0 max-sm:h-10 max-sm:w-10"
                @if ($app_is_demo &&
                        ($category->slug == 'ai_vision' || $category->slug == 'ai_pdf' || $category->slug == 'ai_chat_image')) @else id="send_message_button" @endif type="button">
                <svg class="rtl:-scale-x-100" width="16" height="14" viewBox="0 0 16 14" fill="currentColor"
                    xmlns="http://www.w3.org/2000/svg">
                    <path d="M0.125 14V8.76172L11.375 7.25L0.125 5.73828V0.5L15.875 7.25L0.125 14Z" />
                </svg>
            </button>
            <button class="btn btn-primary hidden h-[52px] w-[52px] shrink-0 rounded-full p-0 max-sm:h-10 max-sm:w-10"
                id="stop_button" type="button">
