...
@foreach ($chat->messages as $message)
    @if ($message->input != null)
        ...
        @if ($message->pdfPath != null && $message->pdfPath != '')
            ...
            <div class="flex flex-row-reverse content-end mb-2 lg:ms-auto gap-[8px] mr-[30px]">
                <a class="flex" href="{{ $message->pdfPath }}">
        ...
        @if ($message->images != null)
            @foreach (explode(',', $message->images) as $image)
                <div class="lqd-chat-image-bubble flex flex-row-reverse content-end mb-2 lg:ms-auto gap-[8px]">
                    <div
                        class="flex justify-end md:w-[50%] w-[80%]  border-none rounded-[2em] mb-[7px]  text-[#090A0A]  dark:text-white">
                        <img class="img-content py-[0.75rem] px-[1.5rem] rounded-[2em]" src={{ $image }} />
                    </div>
                </div>
            @endforeach
        @endif
    ...
    <div class="lqd-chat-ai-bubble flex content-start mb-2 gap-[8px]">
        <span class="text-dark">
            <span class="avatar w-[24px] h-[24px] shrink-0"
                style="background-image: url('{{ !empty($chat->category->image) ? "/".$chat->category->image : asset('assets/img/auth/default-avatar.png') }}')"></span>
        </span>
    ...
    @if ($message->outputImage != null && $message->outputImage != '')
        <div class="lqd-chat-image-bubble flex mb-2 lg:ms-auto gap-[8px]">
            <div
                class="flex justify-end md:w-[50%] w-[80%]  border-none rounded-[2em] mb-[7px]  text-[#090A0A]  dark:text-white">
                <img class="img-content py-[0.75rem] px-[1.5rem] rounded-[2em]" src={{ $message->outputImage }} />
            </div>
        </div>
    @endif
@endforeach
...
@if (count($chat->messages) == 0)
    <div class="flex content-end mb-2">
        <div
            class="border w-full-none rounded-[2em] bg-[#F3E2FD] text-[#090A0A] dark:bg-[rgba(255,255,255,0.02)] dark:text-white">
            <div class="chat-content py-[0.75rem] px-[1.5rem]">
