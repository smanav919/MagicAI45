						...
						<div class="flex flex-col h-full p-[1rem]">
							<div class="flex items-start mb-2 text-[50px] leading-none text-heading font-bold">
								...
								@if(currencyShouldDisplayOnRight(currency()->symbol))
									{{$plan->price}} <small class='inline-flex font-normal text-[0.35em]'>{{ currency()->symbol }}</small>
								@else
								<small class='inline-flex font-normal text-[0.35em]'>{{ currency()->symbol }}</small> {{$plan->price}}
								@endif
							...
							<p class="font-medium text-[15px] leading-none text-muted">{{ __($plan->name) }}</p>
							...
							<div class="mt-auto -mx-[1rem] -mb-[1rem] text-center">
								...
								@if ($plan->price == 0) href = "{{ LaravelLocalization::localizeUrl( route('dashboard.user.payment.startSubscriptionProcess',['planId' => $planid, 'gatewayCode' => 'freeservice' ]) ) }}" @else data-bs-toggle="modal" data-bs-target="#gatewayModal_{{ $planid }}" @endif role="button" >{{__('Choose plan')}}</a>
						...
						<div class="w-full border-solid border-[--tblr-border-color] rounded-3xl {{$plan->is_featured == 1 ? 'shadow-[0_7px_20px_rgba(0,0,0,0.04)]' : ''}}">
							<div class="flex flex-col h-full p-[1rem]">
								...
								<div class="mt-auto -mx-[1rem] -mb-[1rem] text-center">
									...
									@php($planid=$plan->id)
									<a class="btn rounded-3xl py-[0.75rem] -mx-px -mb-px w-full border border-[--tblr-border-color] text-[15px] font-semibold shadow-none hover:bg-[--tblr-primary] hover:text-white" 
									@if ($plan->price == 0) href = "{{ LaravelLocalization::localizeUrl( route('dashboard.user.payment.startPrepaidPaymentProcess',['planId' => $planid, 'gatewayCode' => 'freeservice' ]) ) }}" @else data-bs-toggle="modal" data-bs-target="#gatewayPrepaidModal_{{ $planid }}" @endif  role="button" >{{__('Choose pack')}}</a>
