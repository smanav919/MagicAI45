<header class="site-header absolute inset-x-0 top-0 z-50 text-black transition-[background,shadow] group/header [&.lqd-is-sticky]:bg-white [&.lqd-is-sticky]:shadow-[0_4px_20px_rgba(0,0,0,0.03)] [&.lqd-is-sticky]:text-black">
	<nav id="frontend-local-navbar" class="flex items-center justify-between py-4 border-b border-black px-7 border-opacity-10 relative text-[14px] opacity-0 max-sm:px-2 transition-all duration-500 group-[.page-loaded]/body:opacity-100 group-[.lqd-is-sticky]/header:border-black group-[.lqd-is-sticky]/header:border-opacity-5">
		...
		<div class="flex justify-end gap-2 basis-1/3 max-lg:basis-2/3">
			...
			<div class="hidden md:block relative group">
				...
				<div class="min-w-[145px] bg-white shadow-lg opacity-0 translate-y-2 pointer-events-none absolute top-[calc(100%+1rem)] end-0 rounded-md text-black transition-all group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto">
                    @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
						@if(in_array( $localeCode, explode(',', $setting->languages) ))
			...
			@endif
			<x-button link="{{ LaravelLocalization::localizeUrl( route('login') ) }}" label="{!! __($fSetting->sign_in) !!}" type="outline" border="border-[2px] border-black !border-opacity-10 group-[.lqd-is-sticky]/header:border-black" text="text-black group-[.lqd-is-sticky]/header:text-black group-[.lqd-is-sticky]/header:hover:text-white" />
			<x-button link="{{ LaravelLocalization::localizeUrl( route('register') ) }}" label="{!! __($fSetting->join_hub) !!}" border="border-[2px] border-black !border-opacity-0" bg="bg-black !bg-opacity-10 hover:!bg-opacity-100 group-[.lqd-is-sticky]/header:bg-black group-[.lqd-is-sticky]/header:hover:!bg-opacity-100" text="text-black group-[.lqd-is-sticky]/header:text-black group-[.lqd-is-sticky]/header:hover:text-white" />
