...
function updateChatButtons() {
	setTimeout( function () {
		...
		const generate = async ev => {
			...
			const chatsScroller = $( '.conversation-area' );
			...
			if ( category.slug != 'ai_chat_image' ) {
				...
				$( aiBubbleTemplate.querySelector( '.chat-content-container' ) ).attr( 'class', 'flex items-center bg-white dark:bg-white' );
			...
			if ( pdf != undefined ) {
				const chatPdfBubbleTemplate = document
					.querySelector( '#chat_pdf' )
					.content.cloneNode( true );
				var blob = new Blob( [ pdf ], { type: 'application/pdf' } );
				var url = URL.createObjectURL( blob );
				chatPdfBubbleTemplate.querySelector( '.pdfname' ).innerHTML =
					pdf.name;
				$( chatPdfBubbleTemplate.querySelector( '.pdfpath' ) ).attr(
					'href',
					url
				);
			...
			if ( category.slug == 'ai_chat_image' ) {
				...
				// aiBubbleWrapper.querySelector('.chat-content').innerHTML = response.description;
				chatsScroller[ 0 ].scrollTo( 0, chatsScroller[ 0 ].scrollHeight );
				const chatImageBubbleTemplate = document
			...
			function onWindowScroll() {
				if (
					chatsScroller[ 0 ].scrollTop +
					chatsScroller[ 0 ].offsetHeight >=
			...
			if ( stream_type == 'backend' ) {
				function implementChat( type, images ) {
					...
					const eventSource = new EventSource(
						...
						}&chat_id=${ chat_id }&type=${ type }&images=${ JSON.stringify(
							images
						) }&realtime=${checkked}`
					);
					...
					eventSource.addEventListener( 'data', function ( event ) {
						...
						if ( data.message !== null ) chunk.push( data.message );
					} );
					...
					eventSource.addEventListener( 'error', event => {
						...
						aiBubbleWrapper.querySelector(
							'.chat-content'
						).innerHTML = 'Api Connection Error.';
						eventSource.close();
						clearInterval( nIntervId );
						generateBtn.disabled = false;
						generateBtn.classList.remove( 'submitting' );
