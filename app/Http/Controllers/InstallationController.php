...
";
    ...
    public function installExtension($slug)
    {
        ...
        $url = env('API_SERVER_URL') . "/api/extensions/download?slug=$slug&email=$email";
        ...
        // Open the zip file
        if ($zip->open(Storage::disk('public')->path('file.zip')) === true) {

            error_log("Opened Zip file");

            // Extract the zip file to a temporary directory
            $tmpDir = sys_get_temp_dir() . '/zip-extract';
            ...
            try {
                ...
                $indexJsonPath = $tmpDir . '/index.json';
                $indexJson = file_get_contents($indexJsonPath);

                if (!file_exists(resource_path("extensions/$slug/"))) {
                    mkdir(resource_path("extensions/$slug/"), 0777, true);
                }
                copy($indexJsonPath, resource_path("extensions/$slug/index.json"));
                copy($tmpDir . "/sql/uninstall.sql", resource_path("extensions/$slug/uninstall.sql"));

                $sourcePath = $tmpDir . '/route.php';
                if (!file_exists(base_path("routes/extroutes/"))) {
                    // Create the destination directory
                    mkdir(base_path("routes/extroutes/"), 0777, true);
                }
                error_log(base_path("routes/extroutes/$slug" . "_route.php"));
                copy($sourcePath, base_path("/routes/extroutes/$slug" . "_route.php"));

                $sourcePath = $tmpDir . '/header.blade.php';
                if (!file_exists(resource_path("views/panel/layout/extheaders/"))) {
                    // Create the destination directory
                    mkdir(resource_path("views/panel/layout/extheaders/"), 0777, true);
                }
                copy($sourcePath, resource_path("views/panel/layout/extheaders/$slug" . "_header.blade.php"));

                error_log("Opened index.json file");

                // Decode the JSON data
                $indexData = json_decode($indexJson, true);

                // Copy the files to their respective paths
                foreach ($indexData['files'] as $index => $file) {
                    $sourcePath = $tmpDir . '/files/' . str($index + 1);
                    $destinationPath = base_path($file);
                    error_log("Copying $sourcePath to $destinationPath...");
                ...
                DB::unprepared(file_get_contents($tmpDir . '/sql/install.sql'));

                // Delete the temporary directory
                $filesystem = new Filesystem;
                $filesystem->deleteDirectory($tmpDir);

                // Call Artisan command to clear the cache
                ...
                $filesystem = new Filesystem;
                $filesystem->deleteDirectory($tmpDir);
                return response()->json(['error' => 'Error while installing extension'], 500);
            }
        } else {
            // Return an error message
            return response()->json(['error' => 'Could not open the zip file.'], 500);
        }
    }
    ...
    public function uninstallExtension($slug)
    {
        try {
            // Read the index.json file
            $indexJsonPath = resource_path("extensions/$slug/index.json");
            $uninstallSQL = resource_path("extensions/$slug/uninstall.sql");
            $indexJson = file_get_contents($indexJsonPath);
            ...
            if (file_exists(base_path("/routes/extroutes/$slug" . "_route.php"))) {
                unlink(base_path("/routes/extroutes/$slug" . "_route.php"));
            }

            if (file_exists(resource_path("views/panel/layout/extheaders/$slug" . "_header.blade.php"))) {
                unlink(resource_path("views/panel/layout/extheaders/$slug" . "_header.blade.php"));
            }

            DB::unprepared(file_get_contents($uninstallSQL));

            if (file_exists($indexJson)) {
                unlink($indexJson);
            }
