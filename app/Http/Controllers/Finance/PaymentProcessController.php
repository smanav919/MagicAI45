...
class PaymentProcessController extends Controller
{
    ...
    public function startSubscriptionProcess($planId, $gatewayCode){ # when click on subscribe
    ...
    public function handleWebhook(Request $request, $gateway){
        ...
        try {
            if($request->isMethod('post')){ # accept the post method for all
                if($gateway == "simulate"){
                    return GatewaySelector::selectGateway($gateway)::simulateWebhookEvent();
                }
                return GatewaySelector::selectGateway($gateway)::handleWebhook();
            }
            elseif($request->isMethod('get')) { # accept the get method only for [twocheckout, simulate]
                if($gateway == "simulate"){
                    return GatewaySelector::selectGateway('paypal')::simulateWebhookEvent();
                }
    ...
    public static function checkUnmatchingSubscriptions(){
        ...
        $orders = UserOrder::where([['status', '=', 'Waiting'], ['user_id', '=', auth()->user()->id]])->get();
