...
class PayPalService 
{
    ...
    public static function subscribeCheckout(Request $request, $referral= null)
    {
        ...
        try{
            DB::beginTransaction();
            Log::error(self::$GATEWAY_CODE."-> DB::beginTransaction(); ");
    ...
    public static function getSubscriptionRenewDate(){
        $provider = self::getPaypalProvider();
        $userId=Auth::user()->id;
        $activeSub = Subscriptions::where([['stripe_status', '=', 'active'], ['user_id', '=', $userId]])->orWhere([['stripe_status', '=', 'trialing'], ['user_id', '=', $userId]])->orWhere([['stripe_status', '=', 'paypal_approved'], ['user_id', '=', $userId]])->first();
        if($activeSub != null){
            if($activeSub->stripe_status == "paypal_approved"){
                ...
                if(isset($subscription['error'])){
                    error_log("PayPalService::getSubscriptionStatus() :\n".json_encode($subscription));
    ...
    private static function verifyIncomingJson(Request $request){
        ...
        try{
            ...
            if(json_validate($webhook_event)==false){return false;}
            ...
            error_log("(Webhooks) PayPalService::verifyIncomingJson(): ".$th->getMessage());
        }
